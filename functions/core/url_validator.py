...
class URLValidator:
    """
    URL Validator class for API compatibility.
    Wraps module-level functions in a class interface.
    """

    def is_valid(self, url: str) -> bool:
        """Validate a single URL"""
        return is_valid_property_url(url)
    ...
    def validate_batch(self, urls: List[str]) -> List[dict]:
        ...
        Returns:
            List of dicts with 'url' and 'valid' keys
        """
        results = []
        for url in urls:
            results.append({
                'url': url,
                'valid': is_valid_property_url(url)
            })
        return results
